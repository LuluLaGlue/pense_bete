...
export default function Edit() {
    ...
    const getData = async () => {
        var tmp = data ? null : await axios.get(config.url + config.port + '/data/' + id).catch(err => console.log(err))
        if (tmp && tmp.data) {
            setData(tmp.data);
            setTitle(tmp.data.title);
            setDescription(tmp.data.description)
            setCategory(tmp.data.category);
        }
    }
